<script lang="ts">
	...
	const activeClass = 'bg-surface-200 dark:bg-surface-700 dark:!text-white';
</script>

<div
	role="navigation"
	class="flex flex-col h-full divide-y divide-surface-300 dark:divide-surface-700"
>
	{#if $page.data.user}
		<a
			href="/admin/account"
		...
		>
			<div>
				<Avatar
					width="w-10"
					background="bg-surface-300 dark:bg-surface-500"
		...
		<ul>
			{#each paths as { name, path, icon, subpaths }}
				...
				<li>
					<ul>
						<a
							class="flex items-center px-4 py-2 text-sm text-surface-700 hover:bg-surface-200 dark:hover:bg-surface-700 dark:text-surface-100 dark:hover:text-white {$page
								.url.pathname === path
								? activeClass
								: ''}"
							aria-current={active}
						...
						{#each subpaths as { name, path }}
							...
							<li>
								<a
									class="flex px-4 py-2 text-sm text-surface-400 hover:bg-surface-200 dark:hover:bg-surface-700 dark:text-surface-300 dark:hover:text-white {$page
										.url.pathname === path
										? activeClass
										: ''}"
									aria-current={active}
		...
		<form action="/logout" method="POST" use:enhance>
			<button
				type="submit"
				class="flex items-center w-full px-4 py-2 text-sm text-left text-surface-700 hover:bg-surface-200 dark:hover:bg-surface-700 dark:text-surface-200 dark:hover:text-white"
